<title="Collocation Research - McConnell"><font size = 3>
...
We can also rank pairs and say that certain pairs are more strongly attracted to each other than others. In the field of corpus linguistics, many mathematical measures have been developed that calculate this "attraction". 
...
We asked participants the same questions in two different formats (each participant saw each question in just one format) -- multiple choice (left, below) and typed response (right). Multiple choice questions require the reader to recognize constrained facts that they have already seen while typed questions require them to reconstruct knowledge from the sentence.

![](img_research/tasks.png)

We found that having to answer typed questions lead to people reading more slowly. When people read slower, they seem to have access to statistical information in language for longer... it even lasts over the next couple words they read.
...
We believe this if language is processed more quickly, the statistical information is lost more quickly, because the listener has to deal with all the incoming information. Thus, not only are multiple strategies at play, but speed seems to also have an effect.

## Recap
We asked the question of whether language users take note of statistics such as how often words are used together and if they use these to understand language more quickly and efficiently. We found that this does seem to be the case, though not in a predictive way but rather to assist integration after both words have been read. Further, we found support that commonly co-occurring words may be accessed as larger "chunks". So it seems that comprehenders use both word- and multi-word/chunk-level statistics at the same time. 
