...
However, there are a couple of problems with the measures we have. First of all, they were never developed to tell us about the human mind -- they were developed to tell us about big collections of text samples. The texts included in corpora may not be hte same as the language we hear everyday. Also, listening to language or reading words always involved one word coming before the other. The measures don't usually take this into consideration.

Also, a parallel stream of research suggests that we might not have to think of the two words as separate things at all! It would propose that the mind doesn't work on the level of "words", like we do when we write documents. Instead, the mind will "chunk" words together and see them as one unit, especially if they show up together a lot. This explains why we say "gonna" instead of "going to" -- it's become one chunk! Maybe the same thing happens in our mind for popular combinations of words.

![](img_research/vast_majority2.png)

So we started with two questions: 
1. Do language users keep track of statistics like how often two words are used together?
2. Do language users think on the level of "words" or do commonly used words tend to 'fuse' into larger units in their minds?

## Testing these questions

If words are easier to understand, we tend to read them faster. For this reason, we can tell which words are easier to understand by recording how long it takes them to read them. 

To easily time reading times by word, we had participants read sentences one word at a time. Every time they pressed the spacebar, they could read the next word. 110 English native speakers participated and they each read 248 sentences! 
...
We started sentences very neutrally so that readers couldn't guess what was coming up. Then we included the words 'vast majority' (and 91 other adjective-noun pairs of a similar type) and recorded the reading time to the second word. We wanted to see if reading a word like 'vast' then reading a strongly associated word like 'majority' directly after would speed up reading -- showing that it was easier to read.

## What we found

A lot of factors affect how fast we read, but we weren't interested in them and didn't want to confuse them for the effect we were looking for. Using a statistical model, we removed the influence of word length (we read longer words slower, of course!) and frequency (we read uncommon words slower as well!). We also accounted for the fact that participants may respond differently; some might read faster than others, etc.

Then, we tested to see which measures we had that were the best fit to the pattern of reading times. We were surprised to find that some were very unreliable, although they had are used in different research communities. 

However, two measures of the nine that we tested seemed to fit the data. Interestingly, one of these showed that the second word affects the first word, the opposite direction than we assumed. This suggests that our understanding isn't as predictive as we thought, at least in this case. It may just be the case that expectable words are easier to understand AFTER you've read them
